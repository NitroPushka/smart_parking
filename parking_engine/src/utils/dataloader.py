...
class ParkingDataset(Dataset):
    def __init__(self, dataset_path, transform=None, mode="train", test_size=0.2, random_seed=42):
        ...
        }  # Создает словарь {'free': 0, 'occupied': 1}
    ...
    def _load_and_balance_data(self, test_size, random_state):
        ...
        for class_name in self.class_names:
            ...
            )  # создаем полный путь к папке класса
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.transform:
                ...
                )  # преобразуем массив numpy в тензор PyTorch / меняем оси / во float и преобразовали от 0 до 1
            ...
            dummy_image = (
                torch.zeros((3, 224, 224)) if self.transform else torch.zeros((3, 64, 64))
            )  # Создаем "заглушку" чтобы обучение продолжалось
...
def create_data_loaders(dataset_path, batch_size=32, input_size=224, num_workers=4):
    ...
    train_dataset = ParkingDataset(
        dataset_path=dataset_path, transform=train_transform, mode="train"
    )

    val_dataset = ParkingDataset(dataset_path=dataset_path, transform=val_transform, mode="val")
...
if __name__ == "__main__":
    ...
    try:
        train_loader, val_loader, class_names = create_data_loaders(
            dataset_path=DATASET_PATH, batch_size=16, input_size=224
        )
